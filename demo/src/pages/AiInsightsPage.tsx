  ...
  const handleGenerate = async (actionType: string) => {
    ...
    try {
      ...
      const mockResponses: Record<string, string> = {
        'draft-email': `Subject: Checking in regarding your ${selectedLead.requirement} needs\\n\\nHi ${selectedLead.name},\\n\\nI noticed you are currently looking for solutions related to ${selectedLead.requirement}. Our team at LeadFlow has extensive experience in this area and I'd love to schedule a brief call to discuss how we can help ${selectedLead.company} achieve its goals.\\n\\nBest regards,\\nYour Name`,
        'analyze-sentiment': `Sentiment Analysis for ${selectedLead.name}:\\n\\nBased on the recent interactions and requirements for "${selectedLead.requirement}", the sentiment is overwhelmingly positive. The prospect has shown high intent by actively researching solutions and responding promptly. Recommended action: Fast-track to a demo presentation.`,
        'suggest-next-steps': `Recommended Next Steps:\\n1. Send the tailored case study relevant to ${selectedLead.requirement}.\\n2. Follow up in 2 days if no response.\\n3. Schedule a 15-minute discovery call with the technical lead at ${selectedLead.company}.`
      };

      setGeneratedContent(mockResponses[actionType] || "AI generated insights ready.");
    } catch (error) {
      console.error(error);
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={priorityData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value" stroke="none">
                          {priorityData.map((_entry, index) => <Cell key={`cell-${index}`} fill={PRIORITY_COLORS[index % PRIORITY_COLORS.length]} />)}
                        </Pie>
                        <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', borderRadius: '8px' }} itemStyle={{ color: '#e2e8f0' }} />
                      </PieChart>
                    </ResponsiveContainer>
